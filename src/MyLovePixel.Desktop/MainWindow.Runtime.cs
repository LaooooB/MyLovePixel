...
public sealed partial class MainWindow
{
    private void RefreshPlugins()
    {
        ...
        _pluginsPanel.Children.Add(IconButton("＋", "Load plugin DLL", LoadPluginAsync));
        ...
        _pluginPanelView.SetPanels(_plugins.GetPanels(Current()), (panel, action) =>
        {
            ...
            var result = _plugins.InvokePanelAction(session, panel, action); RefreshAll(); return result;
        });
        _pluginsPanel.Children.Add(_pluginPanelView);
        foreach (var d in _plugins.Diagnostics.TakeLast(6))
        {
            var t = new TextBlock { Text = d, TextWrapping = TextWrapping.Wrap }; t.Classes.Add("subtle"); _pluginsPanel.Children.Add(t);
        }
    }
    ...
    private void RefreshRecovery()
    {
        ...
        foreach (var candidate in candidates.Take(8))
        {
            ...
            if (candidate.IsRecoverable) row.Children.Add(Place(SmallIcon("↺", "Recover", () => RecoverCandidate(candidate.RecoveryId)), 1));
            row.Children.Add(Place(SmallIcon("×", "Dismiss", () => DismissCandidate(candidate.RecoveryId)), 2));
    ...
    private void RefreshTimeline()
    {
        ...
        var session = Current();
        if (session is null) { _timelineStatus.Text = string.Empty; return; }
        ...
        foreach (var frame in window.Items)
        {
            ...
            b.Click += (_, _) => { _playback.Stop(session); session.SelectFrame(frame.Id); };
        ...
        {
            var ms = new NumericUpDown { Value = current.DurationTicks / 1000m, Minimum = 1, Maximum = 60_000, Increment = 1, FormatString = "0" };
            ToolTip.SetTip(ms, "Frame duration ms");
            ms.ValueChanged += (_, _) => { if (ms.Value is { } v) Safe(() => session.SetCurrentFrameDuration((long)(v * 1000m))); };
            _timelineFrames.Children.Add(ms);
        }
    }
    ...
    {
        var session = Current();
        if (session is null) { Title = "MyLovePixel"; _status.Text = string.Empty; return; }
        ...
        var pos = _hover is { } h ? $"{h.X},{h.Y}" : "–";
        _status.Text = $"{pos}   {s.Canvas.Size.Width}×{s.Canvas.Size.Height}   {session.Zoom * 100:0}%{(session.IsDirty ? "   ●" : "")}";
    }
    ...
    private void DispatchCanvasPointer(EditorPointerEvent e)
    {
        ...
        try
        {
            ...
            if (_selectionMode)
            {
                ...
                {
                    _selection.SelectByColor(session, e.CanvasPixel.X, e.CanvasPixel.Y); RefreshCanvas(); return;
                }
                ...
                if (_selectionStart is { } start && e.Kind is EditorPointerKind.Pressed or EditorPointerKind.Moved or EditorPointerKind.Released)
                {
                    ...
                    else if (_selectionGesture == SelectionGestureMode.Ellipse) _selection.SelectEllipse(session, start.X, start.Y, e.CanvasPixel.X, e.CanvasPixel.Y);
                    else _selection.SelectRectangle(session, start.X, start.Y, e.CanvasPixel.X, e.CanvasPixel.Y);
                    RefreshCanvas();
                    if (e.Kind == EditorPointerKind.Released) { _selectionStart = null; _selectionVertices.Clear(); }
                }
                return;
            }
    ...
    {
        var session = Current(); if (session is null) return;
        var c = session.GetToolColors(); session.SetToolColors(c.Secondary, c.Primary);
    }
}
