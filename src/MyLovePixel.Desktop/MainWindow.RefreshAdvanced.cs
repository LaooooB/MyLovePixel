...
public sealed partial class MainWindow
{
    private void RefreshEffects()
    {
        ...
        {
            var row = new Grid { ColumnDefinitions = new ColumnDefinitions("30,*,30,30,30"), ColumnSpacing = 3 };
            row.Children.Add(SmallIcon(effect.Enabled ? "●" : "○", "Enable", () => session.SetEffectEnabled(effect.Id, !effect.Enabled)));
    ...
    private Control BuildEffectParameter(DocumentSession session, EffectInstanceId id, EffectParameterPresentation parameter)
    {
        ...
        row.Children.Add(editor); row.Children.Add(Place(key, 1));
    ...
    private void RefreshTiles()
    {
        ...
        var setCombo = new ComboBox { ItemsSource = setItems, SelectedIndex = Math.Max(0, sets.ToList().FindIndex(v => v.Id == _selectedTileset)) };
        setCombo.SelectionChanged += (_, _) => { if ((uint)setCombo.SelectedIndex < (uint)sets.Count) { _selectedTileset = sets[setCombo.SelectedIndex].Id; _selectedTile = null; RefreshTiles(); } };
        var tw = Number(16, 1, 256); var th = Number(16, 1, 256);
        _tilesPanel.Children.Add(setCombo);
        _tilesPanel.Children.Add(Icons(tw, th, IconButton("＋", "Add tileset", () => { _selectedTileset = session.AddTileset("Tileset", new IntSize((int)(tw.Value ?? 16), (int)(th.Value ?? 16))); RefreshTiles(); })));
        ...
        var tileWrap = new WrapPanel { ItemWidth = 34, ItemHeight = 34 };
        foreach (var tile in tiles.Select((value, index) => (value, index)))
        {
            var b = SmallIcon(tile.index.ToString(), tile.value.Name, () => { _selectedTile = tile.value.Id; _tileErase = false; RefreshTiles(); });
        ...
        var mapCombo = new ComboBox { ItemsSource = maps.Select(v => v.Name).ToArray(), SelectedIndex = Math.Max(0, Array.FindIndex(maps, v => v.Id == _selectedTilemap)) };
        mapCombo.SelectionChanged += (_, _) => { if ((uint)mapCombo.SelectedIndex < (uint)maps.Length) { _selectedTilemap = maps[mapCombo.SelectedIndex].Id; RefreshTiles(); } };
        _tilesPanel.Children.Add(new Grid { ColumnDefinitions = new ColumnDefinitions("*,36"), Children = { mapCombo, Place(IconButton("＋", "Add tilemap", () => { _selectedTilemap = session.AddTilemap("Tilemap", tilesetId); RefreshTiles(); }), 1) } });
        ...
        _tilesPanel.Children.Add(tileTools);
        var vx = Number(_tileViewportX, -8192, 8192); var vy = Number(_tileViewportY, -8192, 8192);
        ...
        _tilesPanel.Children.Add(Icons(vx, vy));
    ...
    private Control BuildTileGrid(DocumentSession session, TilemapId tilemapId)
    {
        ...
        var grid = new Grid();
        for (var i = 0; i < 8; i++) { grid.ColumnDefinitions.Add(new ColumnDefinition(new GridLength(1, GridUnitType.Star))); grid.RowDefinitions.Add(new RowDefinition(GridLength.Auto)); }
        for (var y = 0; y < 8; y++) for (var x = 0; x < 8; x++)
        {
            var cx = _tileViewportX + x; var cy = _tileViewportY + y;
            ...
            Grid.SetColumn(b, x); Grid.SetRow(b, y); grid.Children.Add(b);
        }
        return grid;
    }
    ...
    private void RefreshAnimation()
    {
        ...
        var onion = new StackPanel { Spacing = 4 };
        var prev = Number(_onionPrevious, 0, 12); var next = Number(_onionNext, 0, 12); var opacity = Number(_onionOpacity, 0, 255); var falloff = new NumericUpDown { Value = (decimal)_onionFalloff, Minimum = 0, Maximum = 1, Increment = 0.05m, FormatString = "0.00" };
        ...
        onion.Children.Add(Icons(ToggleIcon("◌", "Onion skin", () => _onionSkin, v => { _onionSkin = v; RefreshCanvas(); RefreshAnimation(); }), prev, next, opacity, falloff));
        _animationPanel.Children.Add(Expander("Onion", onion));

        var clips = new StackPanel { Spacing = 4 };
        clips.Children.Add(IconButton("＋", "Add clip", () => { session.AddAnimationClip($"Clip {session.GetAnimationClips().Count + 1}", 0, s.FrameOrder.Count - 1, AnimationLoopMode.Loop); }));
        foreach (var clip in session.GetAnimationClips()) clips.Children.Add(ListRow($"{clip.Name}  {clip.Start + 1}–{clip.End + 1}", Icons(IconButton("✎", "Edit", async () => await EditClipAsync(clip)), IconButton("×", "Remove", () => session.RemoveAnimationClip(clip.Id)))));
        _animationPanel.Children.Add(Expander("Clip", clips));

        var tags = new StackPanel { Spacing = 4 };
        tags.Children.Add(IconButton("＋", "Add tag at current frame", () => session.AddAnimationTag($"Tag {session.GetAnimationTags().Count + 1}", frameIndex, frameIndex)));
        foreach (var tag in session.GetAnimationTags()) tags.Children.Add(ListRow($"{tag.Name}  {tag.Start + 1}–{tag.End + 1}", Icons(IconButton("✎", "Edit", async () => await EditTagAsync(tag)), IconButton("×", "Remove", () => session.RemoveAnimationTag(tag.Id)))));
        _animationPanel.Children.Add(Expander("Tag", tags));

        var pivot = new StackPanel { Spacing = 4 };
        var px = Number(_hover?.X ?? 0, -8192, 8192); var py = Number(_hover?.Y ?? 0, -8192, 8192);
        pivot.Children.Add(Icons(px, py, IconButton("✓", "Set pivot", () => session.SetPivot((int)(px.Value ?? 0), (int)(py.Value ?? 0))), IconButton("×", "Clear pivot", session.ClearPivot)));
        ...
        boxes.Children.Add(ListRow($"Hitbox · {tracks.HitboxCount}", Icons(IconButton("✎", "Edit hitboxes", EditHitboxesAsync), IconButton("×", "Clear hitboxes", session.ClearHitboxes))));
        boxes.Children.Add(ListRow($"Hurtbox · {tracks.HurtboxCount}", Icons(IconButton("✎", "Edit hurtboxes", EditHurtboxesAsync), IconButton("×", "Clear hurtboxes", session.ClearHurtboxes))));
        _animationPanel.Children.Add(Expander("Boxes", boxes));

        _animationPanel.Children.Add(Expander("Socket", ListRow($"Sockets · {tracks.SocketCount}", Icons(IconButton("✎", "Edit sockets", EditSocketsAsync), IconButton("×", "Clear sockets", session.ClearSockets)))));
        _animationPanel.Children.Add(Expander("Event", ListRow($"Events · {tracks.EventCount}", Icons(IconButton("✎", "Edit events", EditEventsAsync), IconButton("×", "Clear events", session.ClearAnimationEvents)))));

        var slices = new StackPanel { Spacing = 4 };
        slices.Children.Add(IconButton("＋", "Add canvas slice", () => session.AddSpriteSlice($"Slice {session.GetSpriteSlices().Count + 1}", 0, 0, s.Canvas.Size.Width, s.Canvas.Size.Height, s.Canvas.Size.Width / 2, s.Canvas.Size.Height / 2)));
        foreach (var slice in session.GetSpriteSlices()) slices.Children.Add(ListRow(slice.Name, Icons(IconButton("✎", "Edit", async () => await EditSliceAsync(slice)), IconButton("×", "Remove", () => session.RemoveSpriteSlice(slice.Id)))));
        _animationPanel.Children.Add(Expander("Slice", slices));

        if (session.GetPaletteEditors().Count > 0)
        {
            _animationPanel.Children.Add(Expander("Cycle", ListRow($"Cycles · {tracks.ColorCycleCount}", Icons(IconButton("✎", "Edit color cycles", EditColorCyclesAsync), IconButton("×", "Clear cycles", session.ClearColorCycles)))));
        }
    }
}
