...
{
    private readonly MyLovePixel.Application.DocumentSession _session;
    private readonly MyLovePixel.Core.Primitives.TilesetId _tilesetId;
    private readonly MyLovePixel.Core.Primitives.TileId _tileId;
    ...
    public TilePixelDialog(MyLovePixel.Application.DocumentSession session, MyLovePixel.Core.Primitives.TilesetId tilesetId, MyLovePixel.Core.Primitives.TileId tileId)
    {
        _session = session;
        ...
        var bar = new StackPanel { Orientation = Orientation.Horizontal, Spacing = 4, Margin = new Thickness(8) };
        var close = new Button { Content = "✓" }; close.Classes.Add("icon"); close.Classes.Add("primary"); ToolTip.SetTip(close, "Done"); close.Click += (_, _) => Close();
        bar.Children.Add(close);
        DockPanel.SetDock(bar, Dock.Top); root.Children.Add(bar);
        DockPanel.SetDock(_paletteBar, Dock.Top); root.Children.Add(_paletteBar);
        root.Children.Add(new ScrollViewer
        {
            HorizontalScrollBarVisibility = ScrollBarVisibility.Auto,
            VerticalScrollBarVisibility = ScrollBarVisibility.Auto,
            Content = new Border { Background = EditorThemeTokens.CanvasFrame, Margin = new Thickness(12), Padding = new Thickness(12), Child = _view },
        });
        Content = root;
        RefreshView();
    }
    ...
    private void Paint(int x, int y)
    {
        try
        {
            ...
            if (surface.Format == MyLovePixel.Core.Pixel.PixelFormat.Rgba32)
    ...
    private void RefreshView()
    {
        ...
        _paletteBar.IsVisible = surface.Format == MyLovePixel.Core.Pixel.PixelFormat.Indexed8 && palette is not null;
        if (_paletteBar.IsVisible && palette is not null)
        {
            for (var i = 0; i < palette.Count; i++)
            {
                ...
                ToolTip.SetTip(button, $"Index {index}");
...
file sealed class TilePixelEditorView : Control
{
    ...
    public void SetSurface(MyLovePixel.Application.TileSurfacePresentation surface, IReadOnlyList<Rgba32>? palette)
    {
        _surface = surface;
        _palette = palette;
    ...
    protected override void OnPointerPressed(Avalonia.Input.PointerPressedEventArgs e)
    {
        ...
        if ((uint)x < (uint)_surface.Size.Width && (uint)y < (uint)_surface.Size.Height) PixelPressed?.Invoke(x, y);
        e.Handled = true;
    }
}
