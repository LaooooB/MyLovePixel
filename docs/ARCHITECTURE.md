...
未来 Plugin SDK 只能接稳定公开契约，不能获得 Core internal mutable object graph。

### Core

Core 保存文档语义和稳定引用模型。禁止依赖 Avalonia、SkiaSharp、GPU、窗口、文件对话框、具体 UI Widget、具体 Effect evaluator、Recovery。
...
Application 是 UI-facing orchestration boundary。它管理 workspace/session、ActionId、ToolHost routing、file/recovery workflows、snapshot/presentation DTO，并负责把 Command dirty history 转换成 Render invalidation。Application 不得为了 UI 便利绕过 Commands。

### Desktop UI

Desktop 只引用 Application。Avalonia 控件负责窗口、布局、平台输入、timer 和 presentation；不直接访问 mutable `PixelDocument`/`PixelSurface`，不在 click handler 中实现 serializer/export/raster/recovery 算法。
...
5. UI/Application 不得绕过 CommandBus。
...
`DocumentSession` 是受控 live-document 门面；Desktop 只获得 Canvas/Layer/Timeline/Palette/Tool/Recovery presentation 数据。

Canvas pointer 转为 `EditorPointerEvent` 后由 Application 路由 ToolHost。Preview 是 transient overlay，release 才经 CommandBus commit。

Autosave timer 可以由 Desktop 触发，但策略与恢复语义属于 Application/Recovery。Recovery UI 只能调用 Application coordinator。

Diagnostics 包括 Render cache hit/miss/rates、partial/full recompose、texture upload pixels、Undo byte budget/eviction、thumbnail LRU stats。Diagnostics 不进入 `.pixelproj`。
...
插件持久化数据必须 namespace 化；插件缺失时未知 payload 仍需保存。插件不能获取裸 mutable document、GPU lifetime object 或绕过 CommandBus 的写权限。
